...
const i18Obj = {
  ...
  ru: {
    ...
    'soft_skills-1':
      'Коммуникация: У меня развиты навыки эффективного общения, что позволяет успешно взаимодействовать с коллегами и заинтересованными сторонами. Я уделяю особое внимание открытому и ясному обмену информацией, чтобы обеспечить гладкое выполнение работы и успешные результаты проектов.',
    'soft_skills-2':
      'Готовность к обучению: У меня сильное стремление к постоянному обучению и оставанию в курсе последних достижений в быстро развивающейся области технологий. Я активно ищу возможности для расширения своих знаний и навыков, адаптируясь к новым технологиям и изменениям в индустрии разработки.',
    'soft_skills-3':
      'Креативность: Мне нравится генерировать новые идеи и приносить свежий взгляд на разработку проектов. Развитие креативности позволяет мне предлагать уникальные и привлекательные решения, повышая общее качество и пользовательский опыт.',
    'soft_skills-4':
      'Решение проблем: Я активно беру на себя ответственность за любые ошибки и быстро нахожу эффективные решения. Я успешно справляюсь с решением проблем как самостоятельно, так и в команде, обеспечивая оптимальную производительность проекта.',
    'soft_skills-5':
      'Работа в команде: Я командный игрок, ценящий открытую коммуникацию и вклад каждого участника команды. Поддерживая сотрудничество, я активно способствую успеху проектов и общему развитию компании. Я верю, что когда каждый голос учитывается и уважается, мы можем достичь впечатляющих результатов.',
    ...
    'English-text-2':
      'Из-за текущей политической ситуации, возможности для международных поездок и личного общения значительно сократились. Однако, несмотря на это, я сохраняю активное использование английского языка в письменной и устной форме в рамках виртуальных коммуникаций, взаимодействия с международными сообществами и чтения специализированной литературы на английском языке.',
    'English-text-3':
      'Я осознаю важность поддержания и развития своих языковых навыков, поэтому я по-прежнему стремлюсь к самообразованию, изучению английского языка и практике его использования в любой возможности. Я готов быстро восстановить свой уровень знаний и активно принять участие в ситуациях, где требуется общение на английском языке.',
    'English-text-4':
      'Хотя текущая ситуация ограничивает мои возможности для практики английского языка в международных поездках и личных встречах, я остаюсь мотивированным и нацеленным на развитие своих языковых навыков и расширение своего общения на английском языке.',
...
const englishFileName = 'makarov_vladimir.pdf';
const russianFileName = 'макаров_владимир.pdf';

downloadLink.addEventListener('click', function (event) {
  const currentLanguage = window.localStorage.getItem('ru-lang');

  if (currentLanguage === null) {
    localStorage.setItem('ru-lang', 'false');
    downloadFile(event, englishFileName);
    return;
  }

  if (currentLanguage === 'true') {
    downloadFile(event, russianFileName);
    return;
  }
